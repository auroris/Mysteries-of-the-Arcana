...
const endPage = 600;
const increment = 50;
const savePath = path.resolve(__dirname, '../content/posts');

if (!fs.existsSync(savePath)) {
  fs.mkdirSync(savePath, { recursive: true });
}

// Function to clean up author's post by removing avatar section
const cleanAuthorPost = (postHtml) => {
  return postHtml.replace(/<div id="avatar">.*?<\/div>\s*/gi, '').trim();
};

// Function to clean up comment text by removing "Submitted" text and Forum Info span
const cleanCommentText = (commentHtml) => {
  ...
  cleanedText = cleanedText.replace(/<p.*?Submitted.*?<\/p>/gi, '');
...
const fetchBlogPost = async (blogURL, metadata) => {
  try {
    ...
    let postContentHtml = cleanAuthorPost(postContentElement.html().trim());
    ...
    $('.Comment_table .Comment').each((_, commentElement) => {
      ...
      commentTextHtml = cleanCommentText(commentTextHtml);
